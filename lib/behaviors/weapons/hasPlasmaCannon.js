...
module.exports = {
   ...
   update: function updatePlasmaCannon (sprite, game) {
     ...
     // if (game.gamestate.inputs[sprite.name][sprite.megaBlasterControlKey] || sprite.autofire) {
       if (game.time.now > sprite.plasmaCannonTime) {
           ...
           sprite.plasmaCannonTime = game.time.now + BULLET_RATE;
           ...
           Behavior.attach('hasCollisions', bullet, {
             ...
             collisionHandler: function (thing) {
               ...
               if (thing.hardness && thing.hardness > bullet.hardness) {
                 thing.G.health = 0;
                 bullet.G.destroy();
               } else {
                 thing.G.health = 0;
