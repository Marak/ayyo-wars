...
module.exports = {
  ...
  update: function updateHomingMissle (sprite, game) {
    ...
    if (sprite.inputs[sprite.G.homingMissleControlKey]) {
      ...
      // sprite.frame = 3;
      if (sprite.G.homingMissles.length > 3) {
        // return;
      }
      ...
      if (game.time.now > sprite.G.homingMissleTime) {
        ...
        */
        ...
        missle.G.lifespan = HOMING_MISSLE_LIFESPAN;
        // missle.setTexture('missle');

        // missle.body.maxVelocity.set(200);
        sprite.G.homingMissles.push(missle);
        sprite.G.homingMissleTime = game.time.now + HOMING_MISSLE_RATE;
    ...
    sprite.G.homingMissles.forEach(function(m) {
