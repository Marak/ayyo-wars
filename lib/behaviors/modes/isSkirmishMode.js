  ...
  create: function hasSkirmishModeCreate (sprite, opts, game) {
    ...
    ['PLAYER_1', 'PLAYER_2'].forEach(function(p, i){
      ...
      ships.forEach(function(b, i){
        ...
        if (i === 0) {
          sprite.G.squad[p].ships.push({ name: 'MIB Caddy', behavior: b, texture: texture, selected: true });
        } else {
          sprite.G.squad[p].ships.push({ name: 'MIB Caddy', behavior: b, texture: texture });
        }
      });
