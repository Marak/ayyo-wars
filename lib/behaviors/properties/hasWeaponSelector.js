...
module.exports = {
  ...
  update: function hasWeaponSelectorUpdate (sprite, game) {
    if (sprite.inputs) {
      ...
      if (sprite.inputs.action5) {
        weapons.forEach(function(w){
          Behavior.detach(w, sprite);
        });
        Behavior.attach('hasHomingMissle', sprite);
        changedWeapon = 'hasHomingMissle';
      }

      if (sprite.inputs.action6) {
        weapons.forEach(function(w){
          Behavior.detach(w, sprite);
        });
        Behavior.attach('hasEyeOfRa', sprite);
        changedWeapon = 'hasEyeOfRa';
      }
